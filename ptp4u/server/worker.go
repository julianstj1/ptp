...
func (s *sendWorker) Start() {
	...
	for c := range s.queue {
		...
		log.Debugf("Processing client: %s", sockaddrToIP(c.eclisa))
		...
		case ptp.MessageSync:
			...
			log.Tracef("Sending sync %+v to %s from %d", sync, sockaddrToIP(c.eclisa), econn.LocalAddr().(*net.UDPAddr).Port)
			...
			log.Tracef("Sending followup %+v with ts: %s to %s from %d", followup, followup.FollowUpBody.PreciseOriginTimestamp.Time(), sockaddrToIP(c.gclisa), gconn.LocalAddr().(*net.UDPAddr).Port)
		...
		case ptp.MessageAnnounce:
			...
			log.Tracef("Sending announce %+v to %s from %d", announce, sockaddrToIP(c.gclisa), gconn.LocalAddr().(*net.UDPAddr).Port)
