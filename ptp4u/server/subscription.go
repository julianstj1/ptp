...
func (sc *SubscriptionClient) Start() {
	...
	*/
	...
	log.Infof("Starting a new %s subscription for %s", sc.subscriptionType, sockaddrToIP(sc.eclisa))
	log.Debugf("Starting a new %s subscription for %s with load %d with interval %s which expires in %s", sc.subscriptionType, sockaddrToIP(sc.eclisa), l, sc.interval, sc.expire)
	...
	for range intervalTicker.C {
		log.Debugf("Subscription %s for %s is valid until %s", sc.subscriptionType, sockaddrToIP(sc.eclisa), sc.expire)
		if !sc.Running() {
			return
		}
		if time.Now().After(sc.expire) {
			log.Infof("Subscription %s is over for %s", sc.subscriptionType, sockaddrToIP(sc.eclisa))
